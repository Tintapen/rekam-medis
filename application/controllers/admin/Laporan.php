...
class Laporan extends CI_Controller
{
	...
	public function cetak()
	{
		...
		$selectJoin = "SELECT a.tb_soap_id,
		a.tb_reg_pasien_id,
		a.tanggalwaktu,
		a.tb_pasien_id,
		a.tb_dokter_id,
		...
		INNER JOIN tb_dokter c ON a.tb_dokter_id=c.tb_dokter_id";

		if ($this->session->userdata('level') == 'Dokter') {
			$sql = $this->db->query($selectJoin .
				" WHERE a.tb_dokter_id=" . $this->session->userdata('ref_dokter_id') . "
									AND DATE(a.tanggalwaktu) BETWEEN '" . $dari . "' AND '" . $sampai . "'
									ORDER BY tb_soap_id DESC");
		} else {
			$sql = $this->db->query($selectJoin .
				" AND DATE(a.tanggalwaktu) BETWEEN '" . $dari . "' AND '" . $sampai . "'
								ORDER BY tb_soap_id DESC");
		...
		$content .= '</table>
			...
			<tr>
			<td>( ' . ucwords($this->session->userdata('nama')) . ' )</td>
			<td></td>
